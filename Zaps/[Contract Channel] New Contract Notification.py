...
######## End Class Imports #############
### Start Working Code ###
message_title = "A contract has been uploaded for the following:"
message_text = "Name: {name}\nEstimated Value: {value}\nContract Upload Time: {time}".format(
    name=input_data["name"],
    value=input_data["value"] if "value" in input_data else "$0",
    time=input_data["time"]
)

opp_button_action = Action(
    name="opportunity",
    url="https://crm.zoho.com/crm/EntityInfo.do?module=Leads&id={id}".format(id=input_data["id"]),
    text="View Opportunity"
)
attachment = Attachment(
    text=message_text,
    title=message_title,
    actions=[opp_button_action.built_message]
)
message = Message(
    attachments=[attachment.built_message]
)

convo = Conversation(input_data["channel"], token=input_data["bot_token"])
response = convo.post_message(message)
