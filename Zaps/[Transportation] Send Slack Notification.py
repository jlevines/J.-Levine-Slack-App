input_data = {
    'option': 'Call to schedule pick-up',
    'notes': 'These are test notes for transportation',
    'channel_id': 'C7RJBF9T9',
    'won_opp': 'This Is A Test',
    'bot_token': 'xoxb-299198146754-XS7HBPydDnowTHjTQ8kxltVC',
    'id': '2817581000002520017'
}
...
class Conversation:
    ...
    def aim(self):
        ...
        if self.target_type == "channel":
            return self.target
        try:
            return self.get_im(self.target)['id']
        except:
    ...
    def post_message(self, message,
        ...
        if json.loads(response.text)["ok"] == False:
            raise PostMessageError("Slack Error: {}".format(json.loads(response.text)["error"]))
...
######## End Class Imports #############
##### Start Working Code #######
option = input_data['option'] if 'option' in input_data else "N/A"
notes = input_data['notes'] if 'notes' in input_data else "None"

consignment_button = Action(
    name="View Consignment",
    text="View Consignment",
    value="view_consignment",
    url="https://crm.zoho.com/crm/EntityInfo.do?module=Potentials&id={}".format(input_data['id'])
)

transportation_attachment = Attachment(
    callback_id="transpo_needed",
    color="#CF0000",
    actions=[consignment_button.built_message]
)
transportation_attachment.new_field("Won Opportunity Name", input_data['won_opp'],True)
transportation_attachment.new_field("Transportation Option", option,True)
transportation_attachment.new_field("Transportation Notes", notes)

transportation_message = Message(
    text="Transportation Needed For The Following Won Opportunity",
    attachments=[transportation_attachment.built_message]
)

transportation_convo = Conversation(input_data['channel_id'], token=input_data['bot_token'])

response = transportation_convo.post_message(transportation_message)
print("Response: {}".format(response), "Response Text: {}".format(response.text))
