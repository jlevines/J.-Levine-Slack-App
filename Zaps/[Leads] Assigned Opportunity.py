    ...
    if lead_check == True:
        ...
        for x in range(len(users)):
            if users[x]["id"] == lead_details["SMOWNERID"]:
                rep_name = users[x]["content"]
                print("User: {}\n".format(users[x]["content"]))
...
reject_button.new_confirmation("Are you sure?", "If you reject this opportunity, no one else will get it.", "Yes",
                               "No")
...
convo = Conversation(rep_name, target_type="user", token=input_data["bot_token"])
print(rep_name)
print(convo.get_user_by_name(rep_name))
print(convo.aim())
